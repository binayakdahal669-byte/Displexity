...
private:
    ...
    void emitSimpleMain() {
        // Enhanced main that handles Displexity programs
        emit("main:");
        m_indent++;
        emit("mov si, msg_start");
        emit("call print_string");
        emit("");
        emit("; Main program loop");
        emit("mov ax, 42              ; Example: load number");
        emit("call print_number");
        emit("mov si, msg_newline");
        emit("call print_string");
        emit("");
        emit("mov si, msg_done");
        emit("call print_string");
        emit("ret");
        m_indent--;
        emit("");
        emit("msg_start: db 'Displexity OS v1.0', 13, 10, 0");
        emit("msg_done: db 'Program complete. Press any key...', 13, 10, 0");
        emit("msg_newline: db 13, 10, 0");
    }
